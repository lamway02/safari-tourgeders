...
const formspreeEndpoint = 'https://formspree.io/f/mwlkkpdb';
  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section className="inquiry-section" id="inquiry"><div className="container inquiry-grid"><div className="inquiry-copy"><span className="eyebrow">{t('Start a conversation')}</span><h2>{t('Your next chapter')}<br /><em>{t('starts here.')}</em></h2><p>{t('Tell us what you are dreaming about. A real person in Tanzania will get back to you within one working day.')}</p><div className="contact-note"><Mail size={18} /><span>0714547111</span></div></div><form className="inquiry-form" onSubmit={async (event) => { event.preventDefault(); setSubmissionError(false); const response = await fetch(formspreeEndpoint, { method: 'POST', body: new FormData(event.currentTarget), headers: { Accept: 'application/json' } }); if (response.ok) setSubmitted(true); else setSubmissionError(true); }}><label>{t("I'm interested in")} <span>*</span></label><input type="hidden" name="interest" value={activeType} /><div className="choice-grid">{['Safari', 'Kilimanjaro', 'Both'].map((type) => <button type="button" className={activeType === type ? 'choice active' : 'choice'} key={type} onClick={() => setActiveType(type)}>{t(type)}</button>)}</div><div className="form-row"><label>{t('Your name')} <span>*</span><input name="name" required placeholder={t('First and last name')} /></label><label>{t('Email address')} <span>*</span><input name="email" required type="email" placeholder={t('you@example.com')} /></label></div><label>{t('Tell us a little about your plans')}<textarea name="message" placeholder={t('When are you thinking of travelling? Who is coming? What would make this trip yours?')} /></label><button className="button button-gold" type="submit">{t(submitted ? 'Message received' : 'Send inquiry')} <ArrowUpRight size={17} /></button><small>{t(submissionError ? 'Something went wrong. Please try again.' : 'We respect your inbox. No automated itineraries, ever.')}</small></form></div></section>
      </main>
